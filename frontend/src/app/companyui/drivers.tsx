    ...
    async function getDrivers(token: string) {
      ...
      try {
        const response = await axios.get(
          "http://localhost:8080/driver/myDrivers",
          {
            headers: {
  ...
  return (
    <div className="flex flex-row w-full relative ">
          ...
          <div className="flex flex-col gap-3">
            {tabs.map((tab, index) => (
              ...
              >
                {tab.icon}
                <p
                  className={`font-semibold ${tab.key == "drivers" ? "text-blue-600" : "text-gray-600"}`}
                >
                  {tab.label}
                </p>
              </div>
      ...
      <div className="w-full">
        ...
        {showForm && (
          <div
            className="absolute top-32 left-5 md:left-1/2 transform md:-translate-x-1/2 z-50 bg-white border border-gray-300 p-6 rounded-lg shadow-xl"
            ref={registrationRef}
          >
